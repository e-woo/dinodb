...
export const postAnnouncement = async (req, res) => {
  ...
  try {
    ...
    if (titles.includes(title))
      return res
        .status(403)
        .json({ status: 403, message: "Title already exists!" });
    const q2 =
...
export const getAnnouncementExecs = async (req, res) => {
  const { title, isSupervisor } = req.body;
  let q;
  try {
    if (isSupervisor) {
      q = `SELECT Supervisor_ID
			  FROM announcement, supervised_by
			  WHERE announcement.Title = ?
			  AND announcement.Activity_ID = supervised_by.Activity_ID`;
    } else {
      q = `SELECT UCID
...
export const updateAnnouncement = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Announcement updated!", status: 200 });
  } catch (err) {
    return res.status(500).json(err);
  }
};
...
export const deleteAnnouncement = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ status: 200, message: "Succesfully deleted announcement!" });
  } catch (err) {
    return res.status(500).json(err);
  }
};
